...
1. Create a 'data' folder in the root directory of your repository. DONE
1. Inside the 'data' folder, create a 'raw' folder. DONE
1. Add and commit a '.gitignore' file to the root directory of this repository that excludes all contents of the 'data' folder. DONE
1. Download the county-level NRI and SVI data for the entire United States. Place the data in the 'data/raw' folder. DONE
1. In the repository README, provide a brief (1-2 sentence) description of each file in the 'data' folder and a link to the original source of the data. DONE
...
# Confirm it was imported as string
print(nri_data.head())
```
...
```{python}
# Keep only the FIPS code and anything ending in \_AFREQ and \_RISKR
...
print(cross_tab)
```
Counties with missing avalanche frequency data are generally not assigned an avalanche risk rating. When the frequency data is not missing, different risk levels are populated.
...
The discrepancies between the FIPS codes in the NRI and SVI datasets is due to the difference in geographic region covered by each orginization. Specificaly, the NRI dataset includes U.S. territories like American Samoa, Guam, the Northern Mariana Islands, Puerto Rico, and the U.S. Virgin Islands, while the SVI does not. This then means that there are counties in the NRI dataset that are not in the SVI and vice versa. If we merge the data and then aggregate at a state or regional level, missing counties could skew the results. If the counties not covered in both datasets are dropped and they ended up having a high/low risk then the final analysis could be an under/over estimation.

__2. Merge the NRI and SVI data on the FIPS code. Use an outer join to keep all counties in the final dataset.__
```{python }
# Merge the NRI subset and SVI subset using an outer join on the FIPS code to keep all counties from both data sets
...
```{python}
# Same as parts above with new dataset 
...
def plot_histogram(column_name):
    ...
    merged_data[column_name].dropna().hist(bins=30, edgecolor='cyan')
